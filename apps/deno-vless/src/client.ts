async function serveClient(req: Request, basePath: string) {
  ...
  if (url.pathname.startsWith('/assets') || url.pathname.includes(basePath)) {
    ...
    let targetUrl = `https://raw.githubusercontent.com/Juana-blog/v2/cf-worker-vless-poc/apps/cf-page-vless${url.pathname}`;
    if(url.pathname.includes(basePath)){
      targetUrl = `https://raw.githubusercontent.com/Juana-blog/v2/cf-worker-vless-poc/apps/cf-page-vless/index.html`;
    }
